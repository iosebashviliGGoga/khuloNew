...
function PrescentriAnonsebi() {
 ...
 // console.log(languageLink)
  ...
  .then((data) => {
   ...
   // console.log(data)
    setNews(data.ანონსები ?data.ანონსები : {} )
   // console.log(data)
   // console.log('news', news)
    
   // console.log('object keys menu', Object.keys(menu))
   // console.log('object entries news', Object.entries(data.ანონსები))
 ...
 // console.log('sheicvala!')
              ...
              
  {
    ...
    return  <div className="landing-items-container-infos" id={News.id} key={News.id}>
      <Link to={`/anonsebi/${News[1].geo.rec_id}`}>
      <div className='new-image-wrapper'>
        <img src={require('../images/anonsebinew.png')} alt="" />
        </div>
        <div>
    ...
    exit={{opacity: 0}}>
      ...
      <div className="news-container margin-280px">
        <header>
          ...
          <Helmet>
            <title>{language == 1 ?  "ანონსები"  :   ""}{language == 2 ?  "ANNOUNCEMENTS"  :   ""}</title>
          </Helmet>
        </header>
        <div>
...
export default PrescentriAnonsebi