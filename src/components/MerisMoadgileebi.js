...
function MerisMoadgileebi() {
  ...
  const membersList = Object.entries(members).sort((a, b) => b[1].geo.date.localeCompare(a[1].geo.date)).filter(news => news[1].geo.title.includes(search)).map((news) => {
    ...
    if (news[1].geo.m_title?.includes('მერის მოადგილეები') || news[1].geo.m_title?.includes('მერის მოადგილე')) {
      console.log(news[1])
      return <div className="landing-items-container-infos" id={news[0]} key={news[0]}>
        <Link to={`/meris-moadgileebi/${news[0]}`}>
  ...
  return (
    <motion.div
      ...
      <div className="news-container margin-280px">
        <header>
          <span className='big' ref={ref} style={{ marginRight: width / 2 }}> {language == 1 ? "მერია" : "KHULO CITY HALL"}</span>
