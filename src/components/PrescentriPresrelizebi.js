...
function PrescentriPresrelizebi() {
 ...
 // console.log(languageLink)
  ...
  .then((data) => {
   ...
   // console.log(data)
    setNews(data.პრესრელიზები ? data.პრესრელიზები : {})
   // console.log(data)
   // console.log('news', news)
    
   // console.log('object keys menu', Object.keys(menu))
   // console.log('object entries news', Object.entries(data.პრესრელიზები))
 ...
 // console.log('sheicvala!')
              ...
              
  {
    ...
    return  <div className="landing-items-container-infos" id={News.id} key={News.id}>
      <Link to={`/presrelizebi/${News[1].geo.rec_id}`}>
    ...
    exit={{opacity: 0}}>
      ...
      <div className="news-container margin-280px">
        <header>
          ...
          <Helmet>
            <title>{language ==1 ? " პრესრელიზები"  :   ""}
              {language ==2 ? " PRESS RELEASES"  :   ""}</title>
          </Helmet>
        </header>
        <div>
...
export default PrescentriPresrelizebi