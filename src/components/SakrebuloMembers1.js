...
import {FaEnvelope, FaPhoneAlt , FaFacebookF , FaTwitter, FaAngleLeft, FaInstagram, FaInstagramSquare, FaYoutube} from 'react-icons/fa'

function SakrebuloMembers1() {
  ...
  const [loading,setLoading] = useState(true)
    ...
    const memberie = Object.entries(member).length && Object.entries(member).map((item , i) => {
      ...
      <header>
        <span className='big'>{language == 1 ? 'საკრებულო' : "ASSEMBLY"}</span>
        <span className='norm members1' >{menuName  ? ((language == 1 ? menuName[1].name_geo   : menuName[1].name_eng)) : ""  }</span>
        ...
        <div className='sakrebulo-landing sakrebulo1'>
          ...
          <div className='image-wrapper'>
            ...
            <div className="sakrebulo-landing-near-image">
              ...
              <span>{language == 1 ?  member.geo.position : member.eng.title}</span>
              <div className="">
...
export default SakrebuloMembers1