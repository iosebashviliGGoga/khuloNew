...
function KhulosBuneba() {
 ...
 // console.log(languageLink)
  ...
  .then((data) => {
   ...
   // console.log(data)
    setNews(data['კურორტები და ღირსშესანიშნაობები'])
   // console.log(data['კურორტები და ღირსშესანიშნაობები'])
 ...
 //  console.log(Object.entries(data).length)
      ...
      return Object.entries(menu).map((qveItem,index)=>{
       // console.log(qveItem)
        if(qveItem[1].parent_id == item[1].cat_id){
         ...
         // console.log(name)
          return <span key={qveItem[1].name_geo} className={result === slugg? "active" :""}>
  ...
  const newsList = Object.entries(news).length ? Object.entries(news).sort((a, b) => b[1].geo.date.localeCompare(a[1].geo.date)).filter(news => news[1].geo.title.includes(search)).slice(pagesVisited, pagesVisited + usersPerPage).map((News, i) => 
              
  {
              
    return  <div className="landing-items-container-infos" id={News.id} key={News.id}>
      <Link to={`/geografia-da-buneba/${News[1].geo.rec_id}`}>
