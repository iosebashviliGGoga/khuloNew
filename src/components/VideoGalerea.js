...
export default function VideoGalerea() {
  ...
  const newsList = videos.length ? [...Object.entries(videos)].reverse().filter(news => news[1].title_geo.includes(search)).slice(pagesVisited, pagesVisited + usersPerPage).map((news, index) => {
    // console.log(news)
    ...
    return <a href={news[1].youtube_link} target='blank' key={index}> <div className="landing-items-container-infos"  >
      ...
      <div>
        ...
        <span className='vrclad'><Link to={`/news/${news.id}`}>{language == 1 ? " ვრცლად" : ""}
    ...
    <motion.div
      ...
      <div className="news-container margin-280px">
        <header>
          ...
          <span>{language == 1 ? " ვიდეოგალერეა" : ""}
            {language == 2 ? " VIDEO GALLERY" : ""}</span>
          <Helmet>
            <title>{language == 1 ? " ვიდეოგალერეა" : ""}
              {language == 2 ? " VIDEO GALLERY" : ""}</title>
          </Helmet>
        </header>
        <div>

          <div className="landing-container videos">
