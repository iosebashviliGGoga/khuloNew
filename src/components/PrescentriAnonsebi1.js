...
function PrescentriAnonsebi1() {
  ...
  .then((data) => {
   ...
   // console.log('სიახლეები', data.ანონსები)
    setExactNews(data.ანონსები[id])
    setMoreNews(data.ანონსები)
  ...
  {   
      if(i <= 1 ){
          ...
          return  <Link to={`/anonsebi/${news[0]}`}><div className="landing-items-container-infos" id={news.id} key={news.id}>
    ...
    <header>
          ...
          <span>{language ==1 ? " ანონსები"  :   ""}
              {language ==2 ? " ANNOUNCEMENTS"  :   ""}</span>
              <Helmet><title>{language ==1 ? " ანონსები"  :   ""}
              {language ==2 ? " ANNOUNCEMENTS"  :   ""}</title></Helmet>
    </header>
    <div className="main">
        <div>
        ...
        <div className="moreNews">
            <span>{  language == 1 ? "  სხვა ანონსები:" : ""}
                          { language == 2 ? "OTHER ANNOUNCEMENTS:" : ""}</span>
            {newsList}
        </div>
...
export default PrescentriAnonsebi1