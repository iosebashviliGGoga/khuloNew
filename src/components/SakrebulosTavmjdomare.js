...
function  SakrebulosTavmjdomare() {
  ...
  const [loading,setLoading] = useState(true)
    ...
    //  console.log('data', data)
      Object.entries(data).map((item)=>{
        if(item[1].geo.position.includes("თავჯდომარე")){
            setMember(item)
        //    console.log(item)
    ...
    const memberie = Object.entries(member).length && Object.entries(member).map((item , i) => {
      ...
      <header>
        <span className='big' ref={ref} style={{marginRight:  width / 2    }}> {language == 1 ?  "საკრებულო"   :   " "}{language == 2 ?  "KHULO CITY ASSEMBLY"   :   " "}</span>
...
export default SakrebulosTavmjdomare